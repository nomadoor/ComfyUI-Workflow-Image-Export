...
- modal PNG/WebP export saved its preview snapshot, so maximum-edge and
  exceed-mode settings were ignored;
...
full-resolution architecture is tiled.
...
2. Classic selects its renderer only from the final output size. The tiled
   ...
   `downscale` fits the configured maximum edge, while `tile` keeps the native
   resolution and passes no maximum edge to the live Legacy capture.
   ...
   coordinates, and graph tile rectangles use one consistent pixel scale.
...
5. The extension does not register, read, or write its own ComfyUI Settings.
...
- Preview is not treated as the file to save.
