...
import { computeOffscreenBBox, renderGraphOffscreen } from "./render_graph_offscreen.mjs?v=20260903-19";
...
export async function exportWorkflowPng(workflowJson, options = {}) {
  ...
  if (tileEnabled) {
    // Render tiled canvases at their final raster scale. Scaling the assembled
    // 1x canvas would produce the requested dimensions but not 2x detail.
    renderOptions = {
      ...renderOptions,
  ...
  const finalCanvas = await timeSpan(
    perfLog,
    "scale.canvas",
    () => scaleCanvas(canvas, tileEnabled ? 1 : scale)
  );
  if (debug) {
